...
Pressing **Lock This Team** before a deadline creates a versioned projection receipt inside the saved lock: the selected squad/XI/captaincy, all official players' five-event xPts path plus expected-minutes/confidence inputs, and the ranked multi-gameweek transfer evidence and assumptions used at capture time. Player rows use the documented compact `tuple-v4` encoding in `CoachApp.tsx`; it freezes team, position, prior-evidence group, and low-PL-continuity club context for historically correct calibration while `tuple-v1`, `tuple-v2`, and `tuple-v3` receipts remain readable. The compact encoding keeps a full 38-gameweek archive practical in browser storage; signed-in accounts sync the same receipts through the existing D1-backed `locks` field.
...
The History accuracy dashboard aggregates strictly one-gameweek-ahead player calibration across receipts, with exact breakdowns by gameweek, frozen position, frozen club, and prediction confidence (`High ≥ 0.75`, `Medium ≥ 0.50`, otherwise `Low`). It also compares raw captain points and the progressive point swing of frozen transfer routes. Small samples are labelled explicitly and incomplete transfer horizons are excluded rather than treated as zero.
...
Transfer candidates pass through a shared quality gate before ranking. The gate classifies each legal route as **Actionable**, **Watchlist**, or **Blocked** using expected minutes, start probability, confidence, Premier League evidence, club continuity, hard anomaly checks, and whether the five-gameweek gain survives removing its single best week. Only Actionable rows can become the headline recommendation. Watchlist and Blocked rows remain visible for audit, but optimizer utility cannot promote them across the gate. Version-5 projection receipts freeze each route's quality status, score, and reason codes alongside the raw gain evidence.
