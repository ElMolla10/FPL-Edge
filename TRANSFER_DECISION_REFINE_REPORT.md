# Transfer decision refine — type-B HOLD (non-blocking re-entry)

Re-introduced after emergency revert #56 with **non-blocking** scheduling so Overview cannot hang.

See `OVERVIEW_HANG_HOTFIX.md` for hang-prevention details (shallow Overview sync, deferred Transfers deep, budgets, CoachDock open-only).
