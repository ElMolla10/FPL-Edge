...
import { WILDCARD_WEIGHTS } from "./wildcard-tuning";
import { benchOpportunityCostPenalty, buildCaptaincyReferenceByEvent, captaincyEdgeBonus, correlationPenalty, gkStructureTerm, overperformancePenalty, priceFlexibilityBonus } from "./wildcard-scoring";
...
export type WeekPlan={eventId:number;xi:FplPlayer[];bench:FplPlayer[];captain:FplPlayer;vice:FplPlayer;formation:string;points:number;captainPoints:number;vicePoints:number};
...
// Per-term contribution to the objective, for a debug/dev-only breakdown view -- deliberately not
// surfaced anywhere in normal user-facing UI. Every field here is a term already computed inside
// evaluate() for the objective itself; this just exposes them individually instead of only their
// sum, so nothing here can silently drift from what the search actually optimizes.
export type WildcardBreakdown={weightedPoints:number;benchUtility:number;captaincyEdge:number;gkStructure:number;defensiveCorrelation:number;attackingCorrelation:number;benchOpportunityCost:number;overperformance:number;priceFlexibility:number;riskPenalty:number;philosophyBonus:number;objective:number};
export type SquadEvaluation={objective:number;weightedPoints:number;fiveWeekPoints:number;weeks:WeekPlan[];flexibility:number;benchUtility:number;deadSlots:number;riskPenalty:number;bank:number;scores:SquadScores;warnings:string[];strategy:{formation:string;premiums:string[];captain:string;budget:Record<string,number>;benchSpend:number;targets:string[];risk:RiskMode};wildcardBreakdown:WildcardBreakdown;};
...
export function createOptimizer(data:FplData,mode:HorizonMode="Balanced 5 GWs",risk:RiskMode="Balanced",philosophy:SquadPhilosophy="Maximum xPts"){
  ...
  // Precomputed once (not per evaluate() call, which runs thousands of times during annealing) --
  // priceFlexibilityBonus's own pool argument for "realistic one-transfer route" reachability.
  const flatPool=[...poolByPosition.values()].flat();
  // Squad-independent, so computed once per optimizer instance rather than per evaluate() call:
  // tuning is fixed by risk for the whole optimizer, and referenceEliteByEvent (captaincyEdgeBonus's
  // "realistic elite alternative" benchmark) only depends on the pool and the event list, never on
  // which squad is currently being scored.
  const tuning=WILDCARD_WEIGHTS[risk];
  const referenceEliteByEvent=buildCaptaincyReferenceByEvent(flatPool,eventIds,metrics,tuning.captaincyReferencePoolSize);
