...
export type ManagerMeta = { id: number; name: string; teamName: string; overallPoints: number; overallRank: number; gameweekPoints: number; gameweekRank: number; squadValue: number | null; bank: number | null; bankSource?: "live-my-team" | "entry-history" | "unavailable" | null; rankingFinance?: "ready" | "unavailable" | null; publicHistoryBank?: number | null; liveOverlayError?: string | null; transfersMade: number; transferCost: number; /** Live my-team transfers.limit; null = WC/FH or unknown. */ freeTransferLimit?: number | null; captainId: number | null; viceCaptainId: number | null; chip: string | null; event?: number; picks?: OfficialPick[] };
