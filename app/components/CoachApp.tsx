...
function opponent(player:FplPlayer,eventId:number,data:FplData){const fixture=data.fixtures.find(f=>f.event===eventId&&(f.teamH===player.teamId||f.teamA===player.teamId));if(!fixture)return"BLANK";const home=fixture.teamH===player.teamId;const id=home?fixture.teamA:fixture.teamH;return`${data.teams.find(t=>t.id===id)?.short??"—"} ${home?"H":"A"}`}
...
export function bestTransfers(data:FplData,squad:FplPlayer[],bank:number,freeTransfers=1):Transfer[]{
  ...
  for(const out of squad){
    ...
    for(const incoming of data.players){
      ...
      const fixtureGames=events.map(e=>data.fixtures.find(f=>f.event===e.id&&(f.teamH===incoming.teamId||f.teamA===incoming.teamId))).filter(Boolean) as FplFixture[];
      const fixtureAdjustmentIn=fixtureGames.length?fixtureGames.reduce((s,f)=>s+(f.teamH===incoming.teamId?f.teamHDifficulty:f.teamADifficulty),0)/fixtureGames.length:3;
...
function useCaptaincy(players:FplPlayer[],event:number,modelCaptain:FplPlayer|undefined,modelVice:FplPlayer|undefined){
  ...
  useEffect(()=>{if(!event||!players.length)return;let manager:ManagerMeta|null=null;try{manager=JSON.parse(localStorage.getItem("fpl-edge-manager")||"null")}catch{}const valid=(id:number|null|undefined)=>!!id&&players.some(p=>p.id===id);const storedCaptain=Number(localStorage.getItem(`fpl-edge-captain-${event}`));const storedVice=Number(localStorage.getItem(`fpl-edge-vice-${event}`));const nextCaptain=valid(storedCaptain)?storedCaptain:valid(manager?.captainId)?manager!.captainId!:modelCaptain?.id??players[0].id;let nextVice=valid(storedVice)?storedVice:valid(manager?.viceCaptainId)?manager!.viceCaptainId!:modelVice?.id??players.find(p=>p.id!==nextCaptain)?.id??nextCaptain;if(nextVice===nextCaptain)nextVice=players.find(p=>p.id!==nextCaptain)?.id??nextCaptain;setCaptainId(nextCaptain);setViceId(nextVice)},[event,players.map(p=>p.id).join(","),modelCaptain?.id,modelVice?.id]);
...
function PriceIntel({rows}:{rows:Transfer[]}){return <section className="price-intel"><header><span>PRICE-CHANGE INTELLIGENCE</span><h2>Market pressure, without chasing it.</h2></header>{rows.slice(0,4).map(r=>{const net=r.incoming.transfersIn-r.incoming.transfersOut;const level=net>100000?"HIGH":net>30000?"MEDIUM":"LOW";return <article key={r.incoming.id}><b>{r.incoming.name}<small>£{r.incoming.price.toFixed(1)}m</small></b><span>Rise pressure: {level}</span><p>{r.gain5>3?"Transfer case is supported by projected points.":"Price pressure alone does not justify an early move."}</p></article>})}</section>}
...
function Watchlist({data,squad,ids,remove,bank}:{data:FplData;squad:FplPlayer[];ids:number[];remove:(id:number)=>void;bank:number}){
  ...
  <section className="watchlist-grid">{players.length?players.map(p=>{
    ...
    return <article key={p.id}>
      ...
      <footer>
        <div>{events.map(e=>{const fixture=data.fixtures.find(f=>f.event===e.id&&(f.teamH===p.teamId||f.teamA===p.teamId));const difficulty=fixture?(fixture.teamH===p.teamId?fixture.teamHDifficulty:fixture.teamADifficulty):3;return <i key={e.id} className={`fdr-${difficulty}`}>{opponent(p,e.id,data)}<small>{difficulty}</small></i>})}</div>
