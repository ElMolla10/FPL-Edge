...
import { ClubFixtureRow, computeClubFixtureRows, difficultyScoreOutOf10 } from "../lib/fixture-difficulty";
...
function Transfers({data,go,revision,onTeamChange,fullDesk,onUpgrade}:{data:FplData;go:(v:View)=>void;revision:number;onTeamChange:()=>void;fullDesk:boolean;onUpgrade:()=>void}){
  ...
  return <div className="coach-page">
    ...
    {transferTab==="routes"?(fullDesk?<TransferRoutePlanner routes={routes} horizon={routeHorizon} setHorizon={setRouteHorizon} maxWeeklyHit={maxWeeklyHit} setMaxWeeklyHit={setMaxWeeklyHit}/>:<SeasonLocked feature="Multi-week route planner is part of the season pass." onUpgrade={onUpgrade}/>):transferTab==="moves"?<>
      ...
      {fullDesk&&<TransferRouteList title="MAKE & LEAN" eyebrow="SERIOUS MOVES" rows={actionableRows.slice(0,10)} expanded={expanded} toggleExpand={toggleExpand} watchIds={watchIds} setWatch={setWatch} confidence={decisionConfidence} event={a.first} data={data}/>}
      {fullDesk&&<TransferRouteList title="WATCH" eyebrow="WATCH · IMPORTANT" rows={watchlistRows.slice(0,6)} expanded={expanded} toggleExpand={toggleExpand} watchIds={watchIds} setWatch={setWatch} confidence={decisionConfidence} event={a.first} data={data}/>}
      {fullDesk&&<TransferRouteList title="AVOID" eyebrow="AVOID · COLLAPSED" rows={blockedRows.slice(0,6)} expanded={expanded} toggleExpand={toggleExpand} watchIds={watchIds} setWatch={setWatch} confidence={decisionConfidence} event={a.first} data={data}/>}
...
function TransferRouteList({title,eyebrow,rows,expanded,toggleExpand,watchIds,setWatch,confidence,event,data}:{title:string;eyebrow:string;rows:Transfer[];expanded:Set<string>;toggleExpand:(key:string)=>void;watchIds:number[];setWatch:(id:number)=>void;confidence:ReturnType<typeof useTransferDecisionConfidence>;event:number;data:FplData}){
  if(!rows.length)return null;
