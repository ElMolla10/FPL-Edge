...
function Overview({data,go,revision,onTeamChange}:{data:FplData;go:(v:View)=>void;revision:number;onTeamChange:()=>void}){const[meta,setMeta]=useManager(revision);const squad=useMemo(()=>savedSquad(data),[data,revision,meta]);const a=analysis(data,squad);const finance=useMemo(()=>deriveSandboxFinancialContext(squad,data.rules.budget,meta),[squad,data.rules.budget,meta]);
  // Sync shallow only on first paint — never deep type-B beam on the Overview critical path.
  const shallowMoves=useMemo(()=>{if(!a||finance.source==="unavailable"||isRankingFinanceUnavailable(meta))return [] as Transfer[];return bestTransfers(data,squad,finance.baselineBank,authoritativeFreeTransfers(meta),1,finance.baselineSellingPrices,{profile:"overview",mode:"shallow"})},[data,squad,finance,meta,a]);
  ...
  useEffect(()=>{setMoves(shallowMoves);if(!a||finance.source==="unavailable"||isRankingFinanceUnavailable(meta))return;const handle=scheduleDeferred(()=>{
    try{
      const upgraded=bestTransfers(data,squad,finance.baselineBank,authoritativeFreeTransfers(meta),1,finance.baselineSellingPrices,{mode:"deep",rules:{maxEvalCandidates:16,maxPlanNodes:2500,planTimeBudgetMs:120,futureBeamWidth:4,candidatePoolPerPosition:8}});
      setMoves(upgraded);
    }catch{/* keep shallow */}
...
function Transfers({data,go,revision,onTeamChange,fullDesk,onUpgrade}:{data:FplData;go:(v:View)=>void;revision:number;onTeamChange:()=>void;fullDesk:boolean;onUpgrade:()=>void}){
  ...
  useEffect(()=>{
    ...
    const handle=scheduleDeferred(()=>{
      if(cancelled)return;
      try{
        setBaseRows(bestTransfers(data,squad,bank,fts,60,sellingPrices,{mode:"deep"}));
      }catch{
