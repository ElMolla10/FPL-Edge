...
function authoritativeFreeTransfers(meta:ManagerMeta|null|undefined):number{
...
function Transfers({data,go,revision,onTeamChange,fullDesk,onUpgrade}:{data:FplData;go:(v:View)=>void;revision:number;onTeamChange:()=>void;fullDesk:boolean;onUpgrade:()=>void}){
  ...
  },[meta,liveFtKnown,meta?.transfersMade,meta?.freeTransferLimit,meta?.bankSource]);
  ...
  useEffect(()=>{
    ...
    const run=()=>{
      if(cancelled)return;
      try{
        setBaseRows(bestTransfers(data,squad,bank,fts,60,sellingPrices));
      }catch{
        if(!cancelled)setBaseRows([]);
      }
    };
    // Defer heavy type-B engine off first paint so the tab stays interactive.
    const ric=(window as any).requestIdleCallback as undefined|((cb:()=>void,opts?:{timeout:number})=>number);
    const handle=ric?ric(run,{timeout:250}):window.setTimeout(run,0);
    return ()=>{
      cancelled=true;
      if(ric&&typeof handle==="number")(window as any).cancelIdleCallback?.(handle);
      else window.clearTimeout(handle as number);
    };
  },[data,squad,bank,fts,a,sellingPrices,rankingBlocked]);
...
function PitchOutline({onBuild}:{onBuild:()=>void}){
  ...
  return <section className="coach-pitch pitch-outline" aria-label="Empty pitch"><div className="pitch-markings"/>{rows.map(([pos,count])=><div className={`coach-pitch-row ${pos.toLowerCase()}`} key={pos}>{Array.from({length:count},(_,i)=><span className="pitch-slot" key={i}/>)}</div>)}<div className="pitch-empty"><b>No squad yet</b><button type="button" onClick={onBuild}>Build a squad</button></div></section>;
}
