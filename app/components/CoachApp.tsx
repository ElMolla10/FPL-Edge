...
export default function CoachApp({onBack,startAuth=false}:{onBack:()=>void;startAuth?:boolean}){
  ...
  return <TeamLinkAuthProvider value={teamAuth}><main className={desk==="visitor"?"coach-shell signed-out":"coach-shell"}>
    <aside className="coach-sidebar"><button className="brand sidebar-brand" onClick={onBack}><span className="brand-mark">E</span><span>FPL EDGE</span></button><nav className="coach-primary">{sideNav.map(([key,label,icon])=><button key={key} className={view===key?"active":""} onClick={()=>go(key)}><i>{icon}</i><span>{label}</span></button>)}</nav><div className="sidebar-menus"><button type="button" className={proItems.some(([key])=>key===view)?"active":sidebarMenu==="pro"?"open":""} onClick={()=>setSidebarMenu(m=>m==="pro"?null:"pro")}><i>★</i><span>PRO</span></button>{sidebarMenu==="pro"&&<div className="sidebar-drop">{proItems.map(([key,label,icon])=><button key={key} className={view===key?"active":""} onClick={()=>go(key)}><i>{icon}</i><span>{label}</span>{desk!=="season"&&<NavLock/>}</button>)}</div>}<button type="button" className={researchRest.some(([key])=>key===view)?"active":sidebarMenu==="research"?"open":""} onClick={()=>setSidebarMenu(m=>m==="research"?null:"research")}><i>◈</i><span>Research</span></button>{sidebarMenu==="research"&&<div className="sidebar-drop">{researchRest.map(([key,label,icon])=><button key={key} className={view===key?"active":""} onClick={()=>go(key)}><i>{icon}</i><span>{label}</span></button>)}</div>}</div><div className="coach-data-note"><span className={`fresh-dot ${fresh?.tone||"stale"}`}/><div><b>{fresh?`Data ${fresh.label}`:"Connecting…"}</b><small>Official FPL feed</small></div></div><ThemeToggle/><button className="back-link" onClick={onBack}>← Back to site</button></aside>
