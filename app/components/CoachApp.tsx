...
export default function CoachApp({onBack,startAuth=false}:{onBack:()=>void;startAuth?:boolean}){
  ...
  return <TeamLinkAuthProvider value={teamAuth}><main className={desk==="visitor"?"coach-shell signed-out":"coach-shell"}>
    ...
    <nav className="coach-mobile-nav"><button className={view==="overview"?"active":""} onClick={()=>go("overview")}><i>⌂</i>Home</button><button className={mobileOverlay==="My Squad"||inGroup(mySquadGroup)?"active":""} onClick={()=>toggleMobileOverlay("My Squad")}><i>◫</i>My Squad</button><button className={mobileOverlay==="PRO"||proItems.some(([key])=>key===view)?"active":""} onClick={()=>toggleMobileOverlay("PRO")}><i>★</i>PRO</button><button className={view==="coach"?"active":""} onClick={()=>go("coach")}><i>♟</i>Coach</button><button className={mobileOverlay==="More"?"active":""} onClick={()=>toggleMobileOverlay("More")}><i>•••</i>More</button></nav>
    {mobileOverlay==="My Squad"&&<div className="mobile-more">{mySquadGroup.items.filter(([key])=>!PRO_VIEWS.has(key)).map(([key,label,icon])=><button key={key} onClick={()=>go(key)}><i>{icon}</i>{label}</button>)}</div>}
    {mobileOverlay==="PRO"&&<div className="mobile-more">{proItems.map(([key,label,icon])=><button key={key} onClick={()=>go(key)}><i>{icon}</i>{label}{desk!=="season"&&<NavLock/>}</button>)}</div>}
    {mobileOverlay==="More"&&<div className="mobile-more">{navGroups.filter(g=>g.label==="Research"||g.label==="League & History").map(group=><div className="mobile-more-group" key={group.label}><span>{group.label}</span>{group.items.filter(([key])=>!PRO_VIEWS.has(key)).map(([key,label,icon])=><button key={key} onClick={()=>go(key)}><i>{icon}</i>{label}</button>)}</div>)}</div>}
