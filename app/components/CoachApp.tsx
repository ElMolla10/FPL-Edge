...
export default function CoachApp({onBack,startAuth=false}:{onBack:()=>void;startAuth?:boolean}){
  ...
  return <TeamLinkAuthProvider value={teamAuth}><main className={desk==="visitor"?"coach-shell signed-out":"coach-shell"}>
    ...
    <section className="coach-main"><header className="coach-header"><span className="brand header-wordmark"><span className="brand-mark">E</span><span>FPL EDGE</span></span><div className="header-tools">{data&&<DeadlineClock data={data}/>}{desk==="visitor"?<div className="signin-action"><a className="team-signin" href="/signin?return_to=%2F%3Fapp%3D1">Sign in</a><small>Read-only. We never ask for your FPL password.</small></div>:<AccountBar onAuthChange={runSync} onAccount={onAccount} onUpgrade={openPay} initialOpen={startAuth}/>}</div></header>{desk!=="season"&&<p className="pass-banner">You are on the free desk. The season pass is {formatSeasonPassPrice()} and opens the full desk for the rest of this season. <button type="button" onClick={openPay}>Get the season pass</button></p>}
