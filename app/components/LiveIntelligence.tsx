...
import { collectSyncPayload, writeAccountTeam } from "../lib/persistence";
...
export function LiveNews(){
  const{data,error,loading,load}=useOfficialFpl();const[filter,setFilter]=useState("ALL");const[query,setQuery]=useState("");
  // Memoized: this ran on every render before, including every keystroke in the search box and
  // every filter-button click, re-filtering/re-sorting the full player list each time -- matches
  // the pattern LivePlayers already uses correctly for its equivalent computation.
  const owned=useMemo(()=>data?new Set(savedSquad(data).map(p=>p.id)):new Set<number>(),[data]);
  const news=useMemo(()=>data?data.players.filter(p=>p.news||p.status!=="a").filter(p=>(filter==="ALL"||filter==="SQUAD"&&owned.has(p.id)||filter==="DOUBT"&&p.chance!==null||filter==="UNAVAILABLE"&&["i","s","u"].includes(p.status))&&(`${p.name} ${p.teamName} ${p.news}`).toLowerCase().includes(query.toLowerCase())).sort((a,b)=>(b.newsAdded?Date.parse(b.newsAdded):0)-(a.newsAdded?Date.parse(a.newsAdded):0)||b.selectedBy-a.selectedBy):[],[data,filter,query,owned]);
  const movers=useMemo(()=>data?[...data.players].sort((a,b)=>(b.transfersIn-b.transfersOut)-(a.transfersIn-a.transfersOut)).slice(0,5):[],[data]);
  if(!data)return <State loading={loading} error={error} retry={load}/>;
...
export function LiveChips(){
  ...
  // re-ran optimizeSquad() and an 8-event chipScoresForEvent() sweep (which itself calls
  // optimizeSquad() again per event) on every click, even though switching tabs only changes
  // which already-computed row is highlighted, not the underlying model.
  const events=useMemo(()=>data?futureEvents(data,8):[],[data]);
  const stored=useMemo(()=>data?savedSquad(data):[],[data]);
  const hasSquad=useMemo(()=>data?isCompleteSquad(stored,data):false,[data,stored]);
  const baseline=useMemo(()=>data?(hasSquad?stored:optimizeSquad(data,events.slice(0,5).map(e=>e.id))):[],[data,hasSquad,stored,events]);
  const rows=useMemo(()=>data?events.map((event,index)=>({event,...chipScoresForEvent(data,baseline,event,events.slice(index,index+5).map(e=>e.id),hasSquad)})):[],[data,events,baseline,hasSquad]);
  const anomalies=useMemo(()=>data?detectFixtureAnomalies(data):{doubles:[],blanks:[]},[data]);
  const doubleEventIds=useMemo(()=>new Set(anomalies.doubles.map(d=>d.eventId)),[anomalies]);
  const blankEventIds=useMemo(()=>new Set(anomalies.blanks.map(b=>b.eventId)),[anomalies]);
  if(!data)return <State loading={loading} error={error} retry={load}/>;
  const key=selected==="Wildcard"?"wildcard":selected==="Free Hit"?"freeHit":selected==="Bench Boost"?"benchBoost":"tripleCaptain";const best=[...rows].sort((a,b)=>b[key].score-a[key].score)[0];
