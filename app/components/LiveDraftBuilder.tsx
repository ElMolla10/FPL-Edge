...
import { Transfer, bestTransfers, evaluateTransfer, selectPrimaryTransfer } from "../lib/transfers";
...
export type SwapValidation={ok:true}|{ok:false;message:string};
export function validateSwap(squad:Player[],outPlayer:Player,incoming:Player,rules:{budget:number;teamLimit:number}):SwapValidation{
  if(incoming.positionId!==outPlayer.positionId)return{ok:false,message:`${incoming.name} plays a different position and cannot replace ${outPlayer.name}.`};
  const rest=squad.filter(p=>p.id!==outPlayer.id);
  if(rest.some(p=>p.id===incoming.id))return{ok:false,message:`${incoming.name} is already in your squad.`};
  if(rest.filter(p=>p.teamId===incoming.teamId).length>=rules.teamLimit)return{ok:false,message:`Maximum ${rules.teamLimit} players from ${incoming.teamName}.`};
  const costWithoutOut=squad.reduce((s,p)=>s+p.price,0)-outPlayer.price;
  if(costWithoutOut+incoming.price>rules.budget+.001)return{ok:false,message:`That swap exceeds the £${rules.budget.toFixed(1)}m budget.`};
  return{ok:true};
}
...
export default function LiveDraftBuilder({ explorer = false }: { explorer?: boolean }) {
  ...
  // Result of the most recently applied pitch swap, for the summary card + full breakdown toggle.
  // Cleared (not carried forward) by any other squad-changing action so it never describes a swap
  // that's since been superseded -- see remove()/add()/buildBestSquad()/clear-squad/importTeam below.
  const [lastSwap,setLastSwap]=useState<Transfer|null>(null);
  const [swapDetailOpen,setSwapDetailOpen]=useState(false);
  ...
  const buildBestSquad=()=>{
    ...
    setOptimized(nextOptimized);setOptimizedSettings({horizonMode,riskMode,philosophy,resultMode});setSquad(nextOptimized.squad);setSavedAt(null);setSelectedInsight(nextOptimized.squad[0]?.id??null);setSwapOutId(null);setLastSwap(null);setMessage(`${modeLabel} Press Save squad to keep it.`);
  };
  ...
  const swap=(outId:number,incoming:Player)=>{
    ...
    if(!outPlayer)return;
    const validation=validateSwap(squad,outPlayer,incoming,data.rules);
    ...
    const nextSquad=squad.map(p=>p.id===outId?incoming:p);
    const transfer=evaluateTransfer(data,squad,outPlayer,incoming,readFreeTransfers());
    const utilityChange=optimizer.evaluate(nextSquad).objective-optimizer.evaluate(squad).objective;
    setSquad(nextSquad);setSavedAt(null);setSwapOutId(null);setSwapDetailOpen(false);setSelectedInsight(incoming.id);
    setLastSwap({...transfer,utilityChange});
  ...
  const importTeam=async()=>{if(!data)return;if(!/^\d+$/.test(teamId)){setMessage("Enter the numeric Team ID from your official FPL URL.");return}setImporting(true);try{const response=await fetch(`/api/fpl/team?entry=${teamId}`,{cache:"no-store"});const json=await response.json();if(!response.ok)throw new Error(json.error||"Could not import team");const imported:Player[]=json.playerIds.map((id:number)=>data.players.find(p=>p.id===id)).filter(Boolean);if(imported.length!==data.rules.squadSize)throw new Error("FPL did not return a complete 15-player squad.");setSquad(imported);setSavedAt(null);setSwapOutId(null);setLastSwap(null);setMessage(`${json.manager.teamName} imported from GW${json.event}. Review it, then press Save squad.`)}catch(e){setMessage(e instanceof Error?e.message:"Could not import that FPL team.")}finally{setImporting(false)}};
  ...
  return <div className="live-builder">
    ...
    {lastSwap&&<section className="recommended-move pitch-swap-result">
      <div className="call-label"><span>SWAP RESULT</span><b>{lastSwap.qualityStatus==="actionable"?"MODEL EDGE":lastSwap.qualityStatus==="watchlist"?"WATCHLIST":"QUALITY GATE FLAGGED"}</b></div>
      <h2>{lastSwap.out.name} → {lastSwap.incoming.name}</h2>
      <p>{lastSwap.qualityStatus==="actionable"?"The role, evidence and multi-week upside are strong enough to act on.":lastSwap.qualityStatus==="watchlist"?"The upside is interesting, but at least one signal needs more evidence.":"This swap failed a hard plausibility or role-security check."}</p>
      <div>{([["GW","1",lastSwap.gain1],["NEXT","3",lastSwap.gain3],["NEXT","5",lastSwap.gain5]] as const).map(([label,n,value])=><span key={n}><small>{label} {n}</small><b>{value>=0?"+":""}{value.toFixed(1)} pts</b></span>)}<span><small>QUALITY SCORE</small><b>{lastSwap.qualityScore}/100</b></span><span><small>SQUAD RATING</small><b>{lastSwap.utilityChange===null?"—":`${lastSwap.utilityChange>=0?"+":""}${lastSwap.utilityChange.toFixed(1)}`}</b></span></div>
      <button className="swap-detail-toggle" onClick={()=>setSwapDetailOpen(x=>!x)}>{swapDetailOpen?"Hide full breakdown":"Show full breakdown"}</button>
      {swapDetailOpen&&<TransferBreakdown r={lastSwap}/>}
    </section>}
    {complete&&manualEvaluation&&<>
      ...
      <section className={`official-pool ${swapOutId?"swap-mode":""}`}><header><div><span>{explorer?"OFFICIAL PLAYER EXPLORER":swapOutId?"CHOOSE A REPLACEMENT":"CHOOSE A PLAYER"}</span><h2>{explorer?"Every current FPL player and price.":swapOutId?`Replacing ${squad.find(p=>p.id===swapOutId)?.name}.`:"Add or replace anyone."}</h2></div><small>Predictions are FPL Edge estimates, not official FPL forecasts.</small></header><div className="pool-filters"><input value={query} onChange={e=>setQuery(e.target.value)} placeholder="Search by player name…"/><select value={position} onChange={e=>setPosition(e.target.value)}><option value="ALL">All positions</option>{data.rules.positions.map(p=><option value={p.short} key={p.id}>{p.short}</option>)}</select><select value={team} onChange={e=>setTeam(e.target.value)}><option value="ALL">All clubs</option>{teams.map(([id,name])=><option value={id} key={id}>{name}</option>)}</select></div><div className="pool-table"><div className="pool-head"><span>PLAYER</span><span>PRICE</span><span>NEXT GW</span><span>STATUS</span><span>{explorer?"FORM":swapOutId?"REPLACE":"ACTION"}</span></div>{filtered.map(player=><div className="pool-row" key={player.id}><b>{player.name}<small>{player.teamShort} · {player.positionShort}</small></b><span>£{player.price.toFixed(1)}m</span><span>{eventIds.length?playerProjection(player,eventIds[0],data.fixtures,eventIds[0]).toFixed(1):"—"}</span><span className={player.status==="a"?"available":"flagged"}>{player.status==="a"?"Available":player.chance!==null?`${player.chance}% chance`:"Flagged"}</span>{explorer?<span>{player.form.toFixed(1)}</span>:swapOutId?<button disabled={squad.some(p=>p.id===player.id)} onClick={()=>swap(swapOutId,player)}>{squad.some(p=>p.id===player.id)?"Selected":"Swap in →"}</button>:<button disabled={squad.some(p=>p.id===player.id)} onClick={()=>add(player)}>{squad.some(p=>p.id===player.id)?"Selected":"Add +"}</button>}</div>)}</div></section>
  </div>;
}
