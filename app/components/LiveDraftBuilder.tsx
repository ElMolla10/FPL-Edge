...
export default function LiveDraftBuilder({ explorer = false }: { explorer?: boolean }) {
  ...
  const buildBestSquad=()=>{
    ...
    setOptimized(nextOptimized);setOptimizedSettings({horizonMode,riskMode,philosophy,resultMode});setSquad(nextOptimized.squad);setSavedAt(null);setSelectedInsight(nextOptimized.squad[0]?.id??null);setMessage(`${modeLabel} Press Save squad to keep it.`);
  };
  ...
  const remove=(id:number)=>{const player=squad.find(p=>p.id===id);setSquad(x=>x.filter(p=>p.id!==id));setSavedAt(null);setPinnedIds(prev=>prev.has(id)?new Set([...prev].filter(x=>x!==id)):prev);if(player)setMessage(`${player.name} removed. Choose a replacement below.`);};
