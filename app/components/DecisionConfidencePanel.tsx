...
export default function DecisionConfidencePanel({ title, state, candidateLabel, baselineLabel, metricDirection, metricLabel }: {
  title: string;
  ...
  return <section className="decision-confidence-card available" aria-live="polite">
    ...
    <p className="decision-confidence-direction"><b>Signed metrics:</b> {metricDirection}</p>
    <div className="decision-confidence-primary"><article><span>Modeled scenario win rate</span><b>{winRate}</b></article><article><span>Expected {metricLabel} after hit cost</span><b>{signed(result.expectedDelta)} pts</b></article></div>
